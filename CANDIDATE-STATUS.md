...
V0_3_0_TAGGED_RELEASE=NOT_YET_CREATED
...
The v0.3.0 metadata cutover is accepted only after its exact revision passes the configured CI. Linux verification includes complete source-manifest validation, `xtask verify` and the independent byte/numeric/ranking audit. Windows and macOS execute workspace tests and configured runtime checks.
