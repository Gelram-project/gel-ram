...
pub fn invert_view(view: &Orb1024, spec: ViewSpec) -> Result<Orb1024, GelError> {
    match spec.kind {
        ...
        ViewKind::Rotate => apply_view(
            view,
            // Public fields may bypass ViewSpec::rotate; normalize before
            // subtracting so debug/checked builds cannot underflow.
            ViewSpec::rotate(((1024 - (spec.b as usize & 1023)) & 1023) as u16),
        ),
...
/// on the calling thread. Thread-start failure joins started workers, then
/// recomputes the complete result serially. A worker panic can be recovered
/// only in an unwinding build; panic=abort terminates the process. Caller
/// panics and general allocation failure are not covered by this contract.
pub fn top1_threads<'a>(
    bank: &'a [Orb1024],
    query: &Orb1024,
    threads: usize,
) -> Option<(usize, u16, &'a Orb1024)> {
    top1_threads_report(bank, query, threads).0
}

/// Reason the requested parallel path did not produce the final result.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum Top1Fallback {
    BankTooSmall,
    BudgetExceeded,
    ThreadStartFailed,
    WorkerPanicked,
...
/// Per-call execution metadata, not a CPU-utilization measurement.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct Top1Execution {
    pub requested_workers: usize,
    pub worker_budget: usize,
    /// Workers used by the final complete scan, including the caller.
    /// Zero for an empty bank, one after a complete serial retry.
    pub effective_workers: usize,
    /// Successfully started child threads, including discarded partial work.
    pub spawned_workers: usize,
    pub fallback: Option<Top1Fallback>,
}

/// Same answer as [`top1_threads`], with explicit execution/fallback metadata.
pub fn top1_threads_report<'a>(
    bank: &'a [Orb1024],
    query: &Orb1024,
    threads: usize,
) -> (Option<(usize, u16, &'a Orb1024)>, Top1Execution) {
    let scan = |(start, len): (usize, usize)| {
        top1(&bank[start..start + len], query)
            .map(|(index, score, orb)| (start + index, score, orb))
    };
    top1_threads_with_scan(bank.len(), threads, scan, &mut Top1Spawner::default())
}
        ...
        T: Send + 'scope,
    {
        #[cfg(test)]
        {
            ...
            if self.fail_at == Some(attempt) {
                return Err(std::io::Error::other("test-only thread-start refusal"));
            }
        }
...
fn top1_threads_with_scan<'a, F>(
    len: usize,
    threads: usize,
    scan: F,
    spawner: &mut Top1Spawner,
) -> (Option<(usize, u16, &'a Orb1024)>, Top1Execution)
where
    F: Fn((usize, usize)) -> Option<(usize, u16, &'a Orb1024)> + Sync,
{
    let thread_budget = std::thread::available_parallelism()
        .map_or(1, std::num::NonZeroUsize::get)
        .saturating_mul(4)
        .min(256);
    let mut report = Top1Execution {
        requested_workers: threads,
        worker_budget: thread_budget,
        effective_workers: usize::from(len != 0),
        spawned_workers: 0,
        fallback: None,
    };
    if len == 0 {
    ...
    if threads <= 1 || threads > len / 2 || threads > thread_budget {
        if threads > 1 {
            report.fallback = Some(if threads > thread_budget {
                Top1Fallback::BudgetExceeded
            } else {
                Top1Fallback::BankTooSmall
            });
        }
        return (scan((0, len)), report);
    }
    let mut chunks = chunk_bounds(len, threads);
    let last = chunks.next_back().expect("threads >= 2");
    let scan = &scan;
    let parallel = std::thread::scope(|scope| {
        let mut handles = Vec::with_capacity(threads - 1);
        for chunk in chunks {
            match spawner.spawn(scope, move || scan(chunk)) {
        ...
        report.spawned_workers = handles.len();
        // Run the caller's chunk while children can still be scanning.
        // Do not turn a two-worker request into two sequential half-scans.
        let mut best = if report.fallback.is_none() {
            scan(last)
        } else {
            None
        };
        for handle in handles {
            match handle.join() {
                ...
                Ok(None) => {}
                Err(_) => {
                    report.fallback.get_or_insert(Top1Fallback::WorkerPanicked);
                }
            }
        }
        best
    });
    let best = if report.fallback.is_some() {
        // The scope has joined every child before the full serial retry.
        scan((0, len))
    } else {
        report.effective_workers = threads;
        parallel
    };
    (best, report)
}
...
mod tests {
    ...
    #[test]
    fn raw_rotation_fields_roundtrip_for_every_u16() {
        let original = sample(812);
        // Includes 1023, 1024, 1025 and u16::MAX; bypass the constructor.
        for b in 0..=u16::MAX {
            let spec = ViewSpec {
                kind: ViewKind::Rotate,
                a: 1,
            ...
            let rotated = apply_view(&original, spec).unwrap();
            assert_eq!(rotated, apply_view(&original, ViewSpec::rotate(b)).unwrap());
            assert_eq!(invert_view(&rotated, spec).unwrap(), original, "b={b}");
        }
    }
    ...
    #[test]
    fn caller_and_child_scans_overlap_before_join() {
        use std::sync::{Condvar, Mutex};
        use std::time::Duration;
        let bank = random_bank(32, 1201);
        let query = bank[31];
        let rendezvous = (Mutex::new(0usize), Condvar::new());
        let scan = |(start, len)| {
            assert_eq!(len, 16, "unexpected fallback during overlap test");
            ...
            *entered += 1;
            ...
            let (entered, _) = cv
                ...
                .unwrap();
            assert_eq!(*entered, 2, "caller waited for join before scanning");
            top1(&bank[start..start + len], &query).map(|(i, score, orb)| (start + i, score, orb))
        };
        let (got, report) =
            top1_threads_with_scan(bank.len(), 2, scan, &mut Top1Spawner::default());
        assert_eq!(got, top1(&bank, &query));
        assert_eq!(report.spawned_workers, 1);
        assert_eq!(report.effective_workers, 2);
        assert_eq!(report.fallback, None);
    }

    #[test]
    fn start_refusal_joins_partial_work_before_complete_retry() {
        use std::sync::atomic::{AtomicUsize, Ordering};
        for fail_at in 0..3 {
            let bank = random_bank(17, 1202);
            let query = bank[16];
            let completed = AtomicUsize::new(0);
            let full_scans = AtomicUsize::new(0);
            let scan = |(start, len)| {
                if len == bank.len() {
                    assert_eq!(completed.load(Ordering::SeqCst), fail_at);
                    full_scans.fetch_add(1, Ordering::SeqCst);
                }
                let hit = top1(&bank[start..start + len], &query)
                    .map(|(i, score, orb)| (start + i, score, orb));
                if len != bank.len() {
                    completed.fetch_add(1, Ordering::SeqCst);
                }
                hit
            };
            let mut spawner = Top1Spawner {
                fail_at: Some(fail_at),
                attempts: 0,
            };
            let (got, report) = top1_threads_with_scan(bank.len(), 4, scan, &mut spawner);
            assert_eq!(got, top1(&bank, &query));
            assert_eq!(full_scans.load(Ordering::SeqCst), 1);
            assert_eq!(report.spawned_workers, fail_at);
            assert_eq!(report.effective_workers, 1);
            assert_eq!(report.fallback, Some(Top1Fallback::ThreadStartFailed));
        }
    }

    #[test]
    fn execution_report_distinguishes_empty_serial_and_guarded_calls() {
        let bank = random_bank(8, 1203);
        for (len, workers, effective, reason) in [
            (0, 2, 0, None),
            (8, 0, 1, None),
            (8, 1, 1, None),
            (1, 2, 1, Some(Top1Fallback::BankTooSmall)),
            (8, usize::MAX, 1, Some(Top1Fallback::BudgetExceeded)),
        ] {
            let (got, report) = top1_threads_report(&bank[..len], &bank[0], workers);
            assert_eq!(got, top1(&bank[..len], &bank[0]));
            assert_eq!(report.requested_workers, workers);
            assert_eq!(report.effective_workers, effective);
            assert_eq!(report.spawned_workers, 0);
            assert_eq!(report.fallback, reason);
        }
    }

    #[test]
    #[cfg(panic = "unwind")]
    fn worker_panic_retries_only_in_unwinding_builds() {
        let bank = random_bank(16, 1204);
        let scan = |(start, len)| {
            assert!(!(start == 0 && len == 8), "test-only worker panic");
            top1(&bank[start..start + len], &bank[15])
                .map(|(i, score, orb)| (start + i, score, orb))
        };
        let (got, report) = top1_threads_with_scan(16, 2, scan, &mut Top1Spawner::default());
        assert_eq!(got, top1(&bank, &bank[15]));
        assert_eq!(report.effective_workers, 1);
        assert_eq!(report.spawned_workers, 1);
        assert_eq!(report.fallback, Some(Top1Fallback::WorkerPanicked));
    }
