...
1. **Profile:** debug or release, plus the complete `RUSTFLAGS` (see
   [PERFORMANCE.md](PERFORMANCE.md)); no program can print `RUSTFLAGS`.
...
about the tail of a distribution. Two index rules are in use:
`sorted[ceil((n-1)*q)]` (gel-bench, gel-physics) and nearest rank
`sorted[ceil(q*n)-1]` (collection and mutation campaigns). With the first,
p99 is the maximum whenever n < 101; with the second, whenever n < 100.
gel-bench V5 prints `query_p99_is_max=` so the case is explicit.
...
| gel-physics (F0_V3) | yes | no (topology, arch only) | not printed (1) | not printed | not printed | yes |
