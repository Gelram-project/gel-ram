...
- [Live Lab](LIVE-LAB.md) and [source building](SOURCE-BUILDER.md) are the next
  reviewed public integration. Their [update notes](../RELEASE-NOTES-LIVE-LAB.md)
  and [status](../CANDIDATE-STATUS.md) distinguish permission, tests and release tags.
...
1. Integrate source import, exact phrase readout, no-replace save and fresh-process
   reopen in one Rust terminal tool. Test Unicode across storage boundaries,
   invalid pins, truncated/corrupted data and concurrent writers.
2. Run the same reviewed revision on Linux, Windows and macOS. Unix permission
